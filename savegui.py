...
def process_url():
    ...
    # Format and display the new working list
    result_text.insert(END, "#EXTM3U\n")
    for stream in working_streams:
        result_text.insert(END, "#EXTINF:-1 ,{}\n{}\n".format(stream['name'], stream['url']))

def save_playlist():
    playlist = result_text.get(1.0, END)

    # Prompt the user to select a folder for saving the M3U playlist
    folder_path = filedialog.askdirectory()
    if folder_path:
        file_path = f"{folder_path}/working_channels.m3u"
        with open(file_path, "w") as f:
            f.write(playlist)
        save_label.config(text="M3U playlist saved as: {}".format(file_path))
    else:
        save_label.config(text="M3U playlist save cancelled.")
